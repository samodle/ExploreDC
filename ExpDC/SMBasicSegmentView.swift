        ...
        if self.segments.count > 1 {
            ...
            if self.vertical == false {
                ...
                for index in 1..<self.segments.count {
                    //CGPathMoveToPoint(path, nil, originX, 0.0) //old bad swift 3 boo
                    path.move(to: CGPoint(x: originX, y: 0.0))
                    
                    
                    //CGPathAddLineToPoint(path, nil, originX, self.frame.size.height) //old bad swift 3 boo
                ...
                for index in 1..<self.segments.count {
                    //CGPathMoveToPoint(path, nil, 0.0, originY) //swift 3, no longer worxx
                    path.move(to: CGPoint(x: 0.0, y: originY))
                    //CGPathAddLineToPoint(path, nil, self.frame.size.width, originY) //old bad swift 3 boo
